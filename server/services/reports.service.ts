...
import { buildNationalInstitutionalReport } from '@/modules/institutional-reports/builders/national-report.builder.js'
import { buildIncidentInstitutionalReport } from '@/modules/institutional-reports/builders/incident-report.builder.js'
import { buildIncidentDisplayName } from '@/modules/incidents/utils/incident-display-name.js'
...
export async function buildNationalReport(
  ...
  const report: NationalReportDto = {
    ...
    sections: [
      ...
      section('methodology', 'Metodología', 'Agregación determinística de observaciones, eventos, hallazgos y operaciones TerraMind sin narrativa generativa.'),
  ...
  report.institutional = buildNationalInstitutionalReport(
    report,
    includeDemo,
    auth.activeOrganizationId ?? undefined,
  )
  return report
}
...
export async function buildIncidentReport(
  ...
  assertSafeExecutivePayload(report)
  report.institutional = buildIncidentInstitutionalReport(
    report,
    includeDemo,
    auth.activeOrganizationId ?? undefined,
  )
